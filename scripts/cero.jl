...
Correlating Evolutionary Rates of Orthologs
...
while states[:current_step]==2
    diamond_cmd="diamond makedb --in "
    try
        #Make dbs for each species.
        for f in species
            db_name = " -d "f*".db"
            isfile(f*".db.dmnd") || `$diamond_cmd $f $db_name`
        end
        #Focal species to all, all to focal species
        db_focal= " -d "*focal*".db.cmd"
        for f in setdiff(species,focal)
            diamond_cmd = "diamond blastp -f 6 --iterate -k 1 -f "
            db_species= " -d "*f*".db.cmd"
            `$diamond_cmd $f $db_focal`
            `$diamond_cmd $focal $db_species`
        end
    catch
        "Couldn't run diamond! Will write more helpful errors one day."
    end
