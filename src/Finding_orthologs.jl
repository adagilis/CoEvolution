...
"""
function download_and_prep_sequences(taxon;level="chromosome",accession="classic",dir=datadir("seqs/"))
    isdir(dir) || mkdir(dir)
    run(`datasets download genome taxon $taxon --assembly-level $level --dehydrated --include protein,gff3 --filename $dir/$taxon.zip --assembly-source "RefSeq" --mag exclude --exclude-atypical`)
    run(`unzip $dir/$taxon.zip -d $dir`)
    run(`datasets rehydrate --directory $dir`)
    files = filter(contains("GCF"),readdir(dir*"/ncbi_dataset/data/",join=true))
    GCFs = filter(contains("GCF"),readdir(dir*"ncbi_dataset/data/",join=false))
    ...
    run(`rm -rf $dir/ncbi_dataset`)
    run(`rm $dir/README.md`)
    run(`rm $dir/md5sum.txt`)
    run(`rm $dir/$taxon.zip`)
end
...
"""
function diamond_blastp(db,query,seqpath,outpath)
    outname = outpath*db*"_"*query*".blastp.tsv"
    dbname = outpath*db*".dmnd"
    query_seq= seqpath*query*".faa"
    try
        redirect_stderr(devnull)
        isfile(dbname) || diamond_makedb(db,seqpath,outpath)
...
"""
function diamond_makedb(db,seqpath,outpath)
    loc= seqpath*db*".faa"
    out= outpath*db
    ...
    try
        ...
        tprint("{red}Could not make diamond db! Check if properly installed, sequence exists in directory, and new files can be made.{/red}")
    end
end
...
"""
function rbh(species1,species2,seqpath,outpath)
    filename1 = outpath*species1*"_"*species2*".blastp.tsv"
    filename2 = outpath*species2*"_"*species1*".blastp.tsv"
    try
        isfile(filename1) || diamond_blastp(species1,species2,seqpath,outpath)
        isfile(filename2) || diamond_blastp(species2,species1,seqpath,outpath)
...
"""
function find_orths(focal,species_list,seqpath,outpath)
